...
import { LogOut, Plus, Pencil, Trash2, Download, Users, Mail, Phone, MapPin, MessageCircle, Clock, LifeBuoy, FileText, Calendar, Star, DollarSign, Headphones, Settings, Menu, Key, Database, ThumbsUp, ThumbsDown, Lightbulb, ChevronDown, ChevronUp } from "lucide-react";
...
export default function Dashboard({ onLogout, userRole, userPermissions, userChapter, userTitle }) {
  ...
  const handleVoteSuggestion = async (suggestionId, voteType) => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.post(`${API}/suggestions/${suggestionId}/vote`, 
        { vote_type: voteType },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      // Update local state
  ...
  const handleUpdateSuggestionStatus = async (suggestionId, newStatus) => {
    try {
      const token = localStorage.getItem("token");
      await axios.patch(`${API}/suggestions/${suggestionId}/status`, 
        { status: newStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-4 sm:pt-8">
        <div className="bg-slate-800 rounded-xl shadow-sm border border-slate-700 p-4 sm:p-6 mb-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <Lightbulb className="w-5 h-5 text-yellow-400" />
              <h2 className="text-lg font-semibold text-white">Suggestion Box</h2>
            ...
            <div className="flex items-center gap-2">
              <Dialog open={suggestionDialogOpen} onOpenChange={setSuggestionDialogOpen}>
                ...
                <DialogContent className="bg-slate-800 border-slate-700">
                  <DialogHeader>
                    <DialogTitle className="text-white">Submit a Suggestion</DialogTitle>
                  </DialogHeader>
                  <form onSubmit={handleSubmitSuggestion} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-2">
                      ...
                      <Button type="submit" className="bg-yellow-600 hover:bg-yellow-700" data-testid="submit-suggestion-btn">
                        Submit
                      </Button>
                    </div>
                  </form>
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => setSuggestionBoxExpanded(!suggestionBoxExpanded)}
                className="text-slate-400 hover:text-white"
              >
                {suggestionBoxExpanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
              </Button>
            </div>
          </div>

          {suggestionBoxExpanded && (
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex flex-col items-center gap-1 min-w-[50px]">
                        <button
                          ...
                          className={`p-1 rounded hover:bg-slate-600 transition-colors ${
                            suggestion.user_vote === 'upvote' ? 'text-green-400' : 'text-slate-400'
                          }`}
                        ...
                        <button
                          ...
                          className={`p-1 rounded hover:bg-slate-600 transition-colors ${
                            suggestion.user_vote === 'downvote' ? 'text-red-400' : 'text-slate-400'
                          }`}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-slate-300 text-sm mb-2 line-clamp-2">{suggestion.description}</p>
                      ...
                      {(canManageSuggestions || userRole === 'admin') && (
                        <div className="flex items-center gap-1">
                          ...
                          >
                            <SelectTrigger className="w-32 h-8 text-xs bg-slate-600 border-slate-500">
                              <SelectValue />
                            </SelectTrigger>
