...
async def kick_discord_member(member_handle: str, member_id: str, reason: str = "Dues non-payment - 30 days overdue", reason_category: str = None, additional_details: str = None) -> dict:
    ...
    sys.stderr.write(f"🔍 [KICK] Starting kick process for handle='{member_handle}', member_id='{member_id}', category='{reason_category}'\n")
    sys.stderr.flush()
...
"""
                            ...
                            msg = MIMEMultipart('alternative')
                            msg['Subject'] = subject
                            msg['From'] = SUPPORT_SMTP_USERNAME
                            msg['To'] = personal_email
                            msg.attach(MIMEText(text_content, 'plain'))
                            msg.attach(MIMEText(html_content, 'html'))
                            
                            with smtplib.SMTP_SSL(SMTP_HOST, int(SMTP_PORT)) as server:
                                server.login(SUPPORT_SMTP_USERNAME, SUPPORT_SMTP_PASSWORD)
                                server.sendmail(SUPPORT_SMTP_USERNAME, personal_email, msg.as_string())
...
AVAILABLE_PERMISSIONS = [
    ...
    {"key": "manage_nfc", "label": "Manage NFC Cards", "description": "Can program NFC cards for digital business cards"},
]
...
async def get_member_discord_roles(member_id: str, current_user: dict = Depends(verify_token)):
    ...
    member_handle = member.get("handle", "")
    sys.stderr.write(f"[PROMO] GET /discord/member/{member_id}/roles - Looking up handle: {member_handle}\n")
    sys.stderr.flush()
    ...
    try:
        ...
        sys.stderr.write(f"[PROMO] Error getting member roles: {e}\n")
        sys.stderr.flush()
...
if not cors_origins or cors_origins == ['*']:
    cors_origins = [
        "https://themed-login-build.preview.emergentagent.com",
